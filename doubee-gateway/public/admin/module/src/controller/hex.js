layui.define(['treeSelect', 'layer', 'jquery', 'form', 'admin', 'setter', 'table', 'authtree', 'upload', 'laydate'], function (exports) {
    ...
    let hex = {
        ...
        popup(url, fields, done, values = {}, area = '660px', saveTitle = '添加', editTitle = '修改') {
            ...
            layer.open({
                ...
                success: (layero, index) => {
                    fields.forEach(item => {
                        //上传url
                        let uploadUrl = item.hasOwnProperty('uploadUrl') ? item.uploadUrl : '/admin/api/upload/upload';
                        //上传的url字段名称
                                ...
                                if (item.hasOwnProperty('dict')) {
                                    this.getDict(item.dict, res => {
                                        treeSelect.render({
                                            ...
                                            // 数据
                                            data: '/admin/api/dict/getDictionaryByCode?dict=' + item.dict,
        ...
        getDict(dict, done, keywords = '', cache = true) {
            ...
            admin.req({
                url: '/admin/api/dict/getDictionaryByCode',
                type: "get",
