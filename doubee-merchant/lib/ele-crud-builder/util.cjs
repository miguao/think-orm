...
function generateUniqueItemKey(fields, keys) {
  let itemKey = buildCore.generateItemKey();
  const flag = true;
  while (flag) {
    if (!common.findTree(fields, (d) => d.key === itemKey) && (!keys || !keys.includes(itemKey))) {
      break;
    }
    itemKey = buildCore.generateItemKey();
  }
  return itemKey;
}
function itemsGenerateNewKey(fields, curdFields, overwriteProp) {
  ...
  const addedKeys = [];
  common.eachTree(Array.isArray(fields) ? fields : [fields], (item) => {
...
exports.defaultFieldEditFormItems = defaultFieldEditFormItems;
exports.defaultHeaderRightTools = defaultHeaderRightTools;
exports.defaultPageConfigFormItems = defaultPageConfigFormItems;
