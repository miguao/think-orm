...
function getPaginationProps(size, pagination, globalPagination, pageProps) {
  ...
  return Object.assign(defaultProps, common.omit(props, ["autoAmend"]), pageProps);
}
...
function getResponseResult(result, response, globalResponse, lazy, treeOpt) {
  ...
  return {
    data: getResponseData(common.getValue(result, dataName), lazy, treeOpt),
    total: (countName ? common.getValue(result, countName) : void 0) ?? 0,
    result
  };
}
...
function sortData(datasource, sorter) {
  ...
  getOrderItems(sorter).forEach((item) => {
    data.sort((a, b) => {
      ...
      const aValue = common.getValue(a, item.field);
      const bValue = common.getValue(b, item.field);
...
function getCheckedColumns(columns, cols, sortable, isHideInCol, filter, reserveUnchecked, cacheColsWidth) {
  ...
  columns.forEach((d) => {
    ...
    const temp = colId == null ? null : common.findTree(cols, (t) => t.uid === colId);
...
function getColItems(columns, locale, filter, cacheCols, sortFixed, cancelFixed, cacheColsWidth) {
  ...
  const cols = common.mapTree(temp, (d, _i, parent) => {
    const colId = getColId(d);
    if (colId == null || filter && !filter(d)) {
      return;
    }
    const old = common.findTree(cacheCols, (t) => t.uid === colId);
    const item = {
      uid: colId,
      type: d.type,
  ...
  if (cacheCols && cacheCols.length) {
    ...
    common.eachTree(cols, (item) => {
      if (item.children && item.children.length) {
        const oldC = common.findTree(cacheCols, (t) => t.uid === item.uid)?.children;
        if (oldC && oldC.length) {
...
function getCacheColWidth(cacheColsWidth, column) {
  ...
  const c = common.findTree(cacheColsWidth, (d) => d.uid === colId);
  if (!c || c.width == null) {
    return;
  }
...
function getCellText(column, row, index, pageIndex) {
  ...
  const propValue = column.prop == null ? void 0 : common.getValue(row, column.prop);
