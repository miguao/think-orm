...
function isColorValue(value) {
  let color = void 0;
  try {
    color = value ? new fastColor.FastColor(value) : void 0;
    if (color != null) {
...
function getColorLevel(rgb, minus) {
  ...
  return new fastColor.FastColor({ ...rgb, a: Number(a) }).toRgbString();
}
function generateColors(color, dark, isTransparent) {
  const levels = colors.generate(color, dark ? { theme: "dark" } : {});
  const colors$1 = {
    color,
    // 主色
    colorL: levels[4],
  ...
  const rgb = new fastColor.FastColor(color).toRgb();
  return {
    color,
    // 主色
    ...
    // 极亮
    colorD: colors$1.colorD
    // 暗
  };
}
function generateCssVar(color, dark, isTransparent, varName = "primary") {
  const colors2 = generateColors(color, !!dark, !!isTransparent);
  return [
    `--el-color-${varName}:${colors2.color};`,
    `--el-color-${varName}-light-3:${colors2.colorL};`,
    `--el-color-${varName}-light-5:${colors2.colorXL};`,
    `--el-color-${varName}-light-7:${colors2.colorXXL};`,
    `--el-color-${varName}-light-8:${colors2.colorXXXL};`,
    `--el-color-${varName}-light-9:${colors2.colorXXXXL};`,
    `--el-color-${varName}-dark-2:${colors2.colorD};`
  ];
}
