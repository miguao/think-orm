...
const util = require("../ele-viewer/util");
...
function useMobileDevice(onChange) {
  const mobileDevice = vue.ref(false);
...
function useComponentEvents(events, emit) {
  ...
  (Array.isArray(events) ? events : Object.keys(events)).forEach((name) => {
    ...
    emitProps[`on${common.capitalize(name)}`] = (...params) => {
      emit(name, ...params);
    };
  });
  return {
...
function useComponentExpose(componentRef, methodNames, refNames) {
  ...
  refNames.forEach((name) => {
    exposeValues[name] = vue.computed(() => {
      const val = componentRef.value?.[name];
      return val == null ? val : vue.unref(val);
    });
  });
  return exposeValues;
}
function useContentSlot() {
  const customProps = vue.ref();
  const contentRatio = util.useContentRatio((merged) => {
...
function useProOptions(props, name = "options") {
  ...
  const optionsProp = vue.computed(() => common.getValue(props, name));
...
exports.mobileMediaQuery = mobileMediaQuery;
exports.modalItemContextKey = modalItemContextKey;
exports.useCollapseAnim = useCollapseAnim;
exports.useComponentEvents = useComponentEvents;
exports.useComponentExpose = useComponentExpose;
exports.useContentSlot = useContentSlot;
...
exports.useProOptions = useProOptions;
