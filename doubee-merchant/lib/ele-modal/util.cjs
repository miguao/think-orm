...
function getModalContainer(inner, multiple, appendTo, modalsEl) {
  if (multiple) {
    ...
    const wrapper = common.queryChild(parent, containerClass);
    if (wrapper) {
      return wrapper;
    }
...
function getMaxZIndex(modalEl, defaultZIndex) {
  ...
  const currentIndex = common.getCurrentStyle(overlayEl).zIndex;
  ...
  (modals ? Array.from(modals) : []).forEach((modalEl2) => {
    const zIndex = common.getCurrentStyle(modalEl2).zIndex;
    if (zIndex != null) {
