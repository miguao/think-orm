...
function obj2Str(obj, singleLine, indentSize, transformJsVarFunc) {
  ...
  const jsVar = (transformJsVarFunc ?? codeJsVar.transformJsVar)(obj);
...
function generatePropsCode(props, singleLine, indentSize, isAllProps, transformJsVarFunc) {
  ...
  const codes = keys.map((k) => {
    ...
    const jsVar = (transformJsVarFunc ?? codeJsVar.transformJsVar)(props[k]);
    if (jsVar != null && jsVar.name) {
...
exports.addIndentChar = addIndentChar;
exports.escapeStr = escapeStr;
exports.generatePropsCode = generatePropsCode;
