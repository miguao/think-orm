...
import type { ExportPlugin, BeforeExportParams } from './types';

/**
 * 生成表格 HTML
 * @param params 参数
 */
export function getTableHtml(params: BeforeExportParams) {
  ...
  const tableHtml = [
    ...
    '<colgroup>',
    ...bodyCols.map((c) =>
      c.width == null ? '<col/>' : `<col width="${c.width}"/>`
    ),
    '</colgroup>',
    ...(headerData.length
      ? [
          ...
          ...headerData.map((item) =>
            [
              ...
              ...item.map((d) => {
                ...
                const align =
                  d.column?.headerAlign || d.column?.align || 'left';
 ...
 * 导出为 html 格式的插件
 */
export const htmlExportPlugin: ExportPlugin = function (params) {
  return new Promise((resolve) => {
    const tableHtml = getTableHtml(params);
    download(tableHtml, `${params.fileName}.html`, 'text/html;charset=UTF-8');
    resolve();
  });
};
