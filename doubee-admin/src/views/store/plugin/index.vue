  ...
  <ele-page class="plugin-page">
    <div class="page-hero">
      ...
      <div class="page-hero__actions">
        ...
        />
        <el-button class="ele-btn-icon" :icon="ReloadOutlined" @click="reload"
          >刷新</el-button
        >
        <el-button
          class="ele-btn-icon"
      ...
      <div class="filters">
        <div class="filters-left">
          ...
          >
            ...
            <el-option label="安装量" value="installs_desc" />
            <el-option label="评分" value="stars_desc" />
    ...
    <!-- 内容 -->
    <div v-if="viewMode === 'card'" class="plugin-grid">
      ...
      >
        <template #logo="{ row }">
          <div class="table-logo">
            <img :src="row.logo || defaultLogo" alt="" />
          </div>
        </template>
        <template #enabled="{ row }">
          <el-switch
            :model-value="row.enabled"
            @change="(checked: boolean) => togglePlugin(row, checked)"
            size="small"
          />
        </template>
        <template #action="{ row }">
          <el-space :size="8" alignment="center">
            ...
            >
            <el-divider direction="vertical" />
            <el-link type="primary" underline="never" @click="openConfig(row)"
              >配置</el-link
            >
            <el-divider direction="vertical" />
            <el-dropdown @command="(cmd: string) => moreAction(cmd, row)">
    ...
    >
          ...
          <div class="detail__head">
            ...
            <div>
              ...
              <p class="detail__meta">
                ...
                <el-divider direction="vertical" />
                更新：{{ formatTime(detailData?.updatedAt) }}
              </p>
            </div>
          </div>
          <el-alert
      ...
      <template #footer>
        <div class="drawer-actions">
          ...
          <el-button type="primary" @click="openConfig(detailData!)"
            >去配置</el-button
          >
        </div>
      </template>
    ...
    <ele-modal
      v-model="showConfig"
      :title="configData?.name ? `配置：${configData?.name}` : '配置插件'"
    >
    ...
    <!-- 上传插件 -->
    <ele-modal v-model="showUpload" title="上传插件">
      <el-upload
        drag
        action="#"
        :auto-upload="false"
      ...
      >
        ...
        <div class="el-upload__text"
          >将插件包拖到此处，或 <em>点击上传</em></div
        >
        <template #tip>
          <div class="el-upload__tip">支持 zip / tgz，最大 20MB</div>
        </template>
      </el-upload>
      <template #footer>
        <el-button @click="showUpload = false">取消</el-button>
        <el-button type="primary" @click="confirmUpload">开始安装</el-button>
      </template>
    </ele-modal>
  </ele-page>
</template>
...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'StorePlugin' });
