<template>
  ...
  >
    ...
    >
      <el-row :gutter="16">
        <el-col :sm="12" :xs="24">
          <el-form-item label="上级权限" prop="parent_id">
            <permission-select v-model="form.parent_id" />
          </el-form-item>
        ...
        <el-col :sm="12" :xs="24">
          ...
          <el-form-item label="打开方式">
            <el-radio-group
              ...
              :disabled="form.type === 0 || form.type === 2"
        ...
        <el-col :sm="12" :xs="24">
          ...
          <el-form-item prop="path">
            ...
            <el-input
              ...
              v-model="form.path"
              :placeholder="
                form.open_type === 2 ? '请输入外链地址' : '请输入路由地址'
              "
            />
          </el-form-item>
          ...
          <el-form-item prop="component">
            <template #label>
              ...
              <span>{{ form.open_type === 1 ? '内嵌地址' : '组件路径' }}</span>
            </template>
            <el-input
              clearable
              :maxlength="100"
              ...
              :disabled="
                form.type === 0 || form.type === 2 || form.open_type === 2
              "
              :placeholder="
                form.open_type === 1 ? '请输入内嵌地址' : '请输入组件路径'
              "
            />
          </el-form-item>
        </el-col>
      ...
      <el-form-item label="路由元数据" prop="metadata">
        <el-input
          :rows="4"
          type="textarea"
          :maxlength="200"
...
<script lang="ts" setup>
  ...
  import type { Permission } from '@/api/merchant/permission/model';
  import { addPermission, updatePermission } from '@/api/merchant/permission';
  ...
  /** 表单数据 */
  const [form, resetFields, assignFields] = useFormData<Permission>({
    id: void 0,
    parent_id: void 0,
    name: '',
    type: 0,
  ...
  const rules = computed<FormRules>(() => {
    ...
    if (form.type !== 2) {
      obj.path = [
        {
          required: true,
          type: 'string',
          message: form.open_type === 2 ? '请输入外链地址' : '请输入路由地址',
          trigger: 'blur'
        },
        {
          type: 'string',
      ...
      if (form.type === 1 && form.open_type !== 2) {
        obj.component = [
          {
            required: true,
            type: 'string',
            message: form.open_type === 1 ? '请输入内嵌地址' : '请输入组件路径',
            trigger: 'blur'
          },
          {
            type: 'string',
    ...
    if (visible.value) {
      if (props.data) {
        ...
        assignFields({
          ...
          parentId: props.data.parent_id === 0 ? void 0 : props.data.parent_id
        });
        isUpdate.value = true;
      } else {
        resetFields();
